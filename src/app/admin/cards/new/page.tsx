...
export default function NewCardPage() {
  ...
  const handleSave = async () => {
    if (!form.name.trim()||!form.base_price||!form.category||!form.description.trim()) { showToast('Fill all required fields','error'); return; }
    setSaving(true);
    try {
      const payload = { ...form, images:form.images.filter(u=>u.trim()!==''), original_price:form.original_price||undefined, short_video_url:form.short_video_url?.trim()||undefined, card_code:form.card_code?.trim()||undefined, meta_title:form.meta_title?.trim()||'', meta_description:form.meta_description?.trim()||'', image_alt_text:form.image_alt_text?.trim()||'' };
  ...
  const setAddOn=(i:number,k:keyof AddOn,v:string|number)=>setForm(f=>{const a=[...f.add_ons];a[i]={...a[i],[k]:v};return{...f,add_ons:a};});
  const addAddOn=()=>setForm(f=>({...f,add_ons:[...f.add_ons,{name:'',price:0,description:''}]}));
  const removeAddOn=(i:number)=>setForm(f=>({...f,add_ons:f.add_ons.filter((_,j)=>j!==i)}));

  return (
    <>
      <Script src="https://upload-widget.cloudinary.com/global/all.js" strategy="lazyOnload" onLoad={()=>{cloudinaryLoaded.current=true;}} />
      ...
      <div className="ec-page">
        <header className="ec-header">
          ...
          <h1 className="ec-title">Add New Card</h1>
          <button className="ec-save-btn" onClick={handleSave} disabled={saving}>{saving?'Saving…':'Create Card'}</button>
        </header>
          ...
          <div className="ec-form">
            <div className="ec-grid2">
              <div className="ec-field">
                <label className="ec-label">Card Name <span className="ec-req">*</span></label>
                <input className="ec-input" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value,slug:toSlug(e.target.value)}))} placeholder="e.g. Royal Mughal Velvet" />
              </div>
              <div className="ec-field">
                <label className="ec-label">Card Code</label>
                <input className="ec-input" value={form.card_code} onChange={e=>setForm(f=>({...f,card_code:e.target.value}))} placeholder="e.g. WC-001" />
              </div>
            </div>

            <div className="ec-field">
              <label className="ec-label">Slug <span className="ec-req">*</span></label>
              <input className="ec-input ec-input--mono" value={form.slug} onChange={e=>setForm(f=>({...f,slug:e.target.value.toLowerCase().replace(/\s+/g,'-')}))} />
              <p className="ec-hint">URL: /product/{form.slug||'…'}</p>
            </div>

            <div className="ec-grid2">
              <div className="ec-field">
                <label className="ec-label">Category <span className="ec-req">*</span></label>
                <select className="ec-select" value={form.category} onChange={e=>setForm(f=>({...f,category:e.target.value}))}>
                  {CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div className="ec-field">
                <label className="ec-label">Min Order (pcs)</label>
                <input className="ec-input" type="number" min={1} value={form.min_order} onChange={e=>setForm(f=>({...f,min_order:Number(e.target.value)}))} />
              </div>
            </div>

            <div className="ec-grid2">
              <div className="ec-field">
                <label className="ec-label">Base Price (PKR/card) <span className="ec-req">*</span></label>
                <input className="ec-input" type="number" min={0} value={form.base_price||''} onChange={e=>setForm(f=>({...f,base_price:Number(e.target.value)}))} placeholder="e.g. 350" />
              </div>
              <div className="ec-field">
            ...
            <div className="ec-field">
              <label className="ec-label">Description <span className="ec-req">*</span></label>
              <RichTextEditor value={form.description} onChange={html=>setForm(f=>({...f,description:html}))} placeholder="Brief description of the card…" maxLength={5000} minRows={5} />
            </div>

            <div className="ec-field">
              <div className="ec-field-header">
                <label className="ec-label">Card Images</label>
                <button type="button" className="ec-btn-sm" onClick={()=>setShowUrls(v=>!v)}>{showUrls?'Hide URLs':'Paste URL'}</button>
              </div>
              <button type="button" className="ec-upload-zone" onClick={()=>openWidget('image')}>
                ...
                <span className="ec-upload-hint">JPG, PNG, WebP — max 10 MB</span>
              </button>
              {form.images.filter(u=>u.trim()).length>0 && (
                <div className="ec-img-grid">
                  {form.images.map((url,i)=>url.trim()?(
                    <div key={i} className="ec-img-card">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={url} alt="" className="ec-img-card__img" onError={e=>{e.currentTarget.style.display='none';}} />
                      <button type="button" className="ec-img-card__remove" onClick={()=>removeImage(i)}>✕</button>
                    </div>
                  ):null)}
                </div>
              )}
                ...
                <div className="ec-url-list">
                  {form.images.map((url,i)=>(
                    <div key={i} className="ec-url-row">
                      <input type="url" className="ec-input" value={url} onChange={e=>setImage(i,e.target.value)} placeholder="https://res.cloudinary.com/…" />
                      {form.images.length>1 && <button type="button" className="ec-btn-danger" onClick={()=>removeImage(i)}>✕</button>}
                    </div>
                  ))}
                  <button type="button" className="ec-btn-sm" onClick={addImage}>+ Add URL</button>
                </div>
              )}
            </div>

            <div className="ec-field">
              <label className="ec-label">Short Preview Video <span className="ec-opt">(optional, ~5 sec)</span></label>
              {form.short_video_url ? (
                <div className="ec-video-wrap">
                  <video src={form.short_video_url} className="ec-video" controls muted playsInline loop />
                  <div className="ec-video-actions">
                    <button type="button" className="ec-btn-sm" onClick={()=>openWidget('video')}>Replace</button>
                    <button type="button" className="ec-btn-danger" onClick={()=>setForm(f=>({...f,short_video_url:''}))}>✕ Remove</button>
                  </div>
                </div>
              ) : (
                <button type="button" className="ec-upload-zone" onClick={()=>openWidget('video')}>
            ...
            <div className="ec-field">
              <label className="ec-label">SEO Meta Description <span className="ec-opt">(optional)</span></label>
              <textarea className="ec-input" style={{resize:'vertical',minHeight:'80px'}} value={form.meta_description||''} onChange={e=>setForm(f=>({...f,meta_description:e.target.value}))} placeholder="Brief summary for search engines…" />
            </div>

            <div className="ec-field">
              <label className="ec-label">Image Alt Text <span className="ec-opt">(optional)</span></label>
              <input className="ec-input" value={form.image_alt_text||''} onChange={e=>setForm(f=>({...f,image_alt_text:e.target.value}))} placeholder="e.g. Red velvet wedding card with gold foil" />
            </div>

            <div className="ec-field">
              <div className="ec-field-header">
                <label className="ec-label">Add-ons</label>
                <button type="button" className="ec-btn-sm" onClick={addAddOn}>+ Add Row</button>
              </div>
              {form.add_ons.length===0 && <p className="ec-hint">No add-ons yet.</p>}
              {form.add_ons.map((ao,i)=>(
                <div key={i} className="ec-addon-row">
                  <input className="ec-input" placeholder="Name (e.g. Gold Foil)" value={ao.name} onChange={e=>setAddOn(i,'name',e.target.value)} />
                  <input className="ec-input ec-input--sm" type="number" placeholder="Price" min={0} value={ao.price} onChange={e=>setAddOn(i,'price',Number(e.target.value))} />
                  <input className="ec-input" placeholder="Description" value={ao.description} onChange={e=>setAddOn(i,'description',e.target.value)} />
                  <button type="button" className="ec-btn-danger" onClick={()=>removeAddOn(i)}>✕</button>
                </div>
              ))}
            </div>
