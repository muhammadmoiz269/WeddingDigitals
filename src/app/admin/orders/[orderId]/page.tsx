...
const STATUS_OPTIONS = [
  ...
  { value: 'out_for_delivery', label: '🚚 Out for Delivery', color: '#14b8a6' },
...
function statusColor(s: string) {
  return STATUS_OPTIONS.find(o => o.value === s)?.color ?? LEGACY_STATUSES[s]?.color ?? '#6b7280';
}
...
function buildWhatsApp(order: any) {
  const msg =
    ...
    (order.discount?.amount
      ? `🎁 *Discount:* ${order.discount.source === 'promo' ? order.discount.code : 'Bulk discount'} − PKR ${order.discount.amount.toLocaleString()}\n`
      : '') +
          ...
          <Section title="Payment" icon="💳">
            <div className="od-fields">
              ...
              {order.discount?.amount > 0 && (
                <>
                  <Field label="Subtotal" value={`PKR ${(order.subtotal_before_discount ?? order.total + order.discount.amount)?.toLocaleString()}`} />
                  <Field
                    label={order.discount.source === 'promo' ? `Promo (${order.discount.code})` : 'Bulk Discount'}
                    value={`− PKR ${order.discount.amount?.toLocaleString()}`}
                  />
                </>
              )}
              <Field label="Order Total" value={`PKR ${order.total?.toLocaleString()}`} highlight />
              <Field label="Amount Due" value={order.payment?.amount_due != null ? `PKR ${order.payment.amount_due.toLocaleString()}` : undefined} />
            </div>
